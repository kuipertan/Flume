...
public class SpillableMemoryChannel extends FileChannel {
  ...
  public static final String DURABLE_FILE = "durableFile";
  ...
  public void configure(Context context) {
    ...
    durableFile = context.getString(DURABLE_FILE, "").trim();
    scenarioFile = context.getString(SCENARIO_FILE, "").trim();
    LOGGER.info("durable file:" + durableFile);
  ...
  private int recoverMem(){
    try{
      ArrayDeque<MutableInteger> queue = new ArrayDeque<MutableInteger>(1000);
      File file = new File(scenarioFile); 
      BufferedReader reader  = new BufferedReader(new FileReader(file));
      String line = reader.readLine();  
      while(line != null){    
        int n = Integer.parseInt(line.replaceAll("\n", ""));  
        queue.addLast(new MutableInteger(n));
        line = reader.readLine();
      }  
      reader.close();
      drainOrder.queue = queue;
      if(queue.size() == 0)
        return 0;

      int eventByteSize = 0;
      FileInputStream fis = new FileInputStream(durableFile);
      ObjectInputStream ois = new ObjectInputStream(fis);
      while(true){
        SerializableEvent ev = (SerializableEvent) ois.readObject();
       	if(ev == null){ 
            break;
        }
        LOGGER.info("recover event:" + ev.toString());     
        Event sev = EventBuilder.withBody(ev.getBody(), ev.getHeaders()); 
        memQueue.offer(sev);
        eventByteSize += (int)Math.ceil(estimateEventSize(sev)/ avgEventSize);
      }
      ois.close();
      memQueRemaining.acquire(memQueue.size());
      bytesRemaining.acquire(eventByteSize);
    }
    catch(Exception ex){
      LOGGER.error(ex.toString());
      drainOrder.queue.clear();
      memQueue.clear();
    }
    return drainOrder.queue.size();
  }

  private void storeScenario(){
    File file = new File(scenarioFile);
    ArrayDeque<MutableInteger> queue = new ArrayDeque<MutableInteger>(drainOrder.queue);
    try{
      BufferedWriter writer  = new BufferedWriter(new FileWriter(file));
      while(true){
        MutableInteger mi = queue.poll();
        if(mi == null)
          break;
        writer.write(mi.intValue() + "\n");
      }
      writer.flush();
      writer.close(); 
    }
  ...
  private void storeMem(){
    try{
      FileOutputStream fos = new FileOutputStream(durableFile);
      ObjectOutputStream oos = new ObjectOutputStream(fos);
      
      while(true){
        Event ev = memQueue.poll();
        if(ev == null)
          break;
         SerializableEvent sev = new SerializableEvent(ev);
        oos.writeObject(sev);
      }
  ...
  public synchronized void start() {
    ...
    if (drainOrder.isEmpty()) {
      ...
      if(sz == 0){
        ...
        totalStored.release(overFlowCount + memQueue.size());
    }
  ...
  public synchronized void stop() {
    ...
    storeMem();
    LOGGER.info("stop when memQueue size=" + memQueue.size() + " overflow=" + super.getDepth());
    channelCounter.setChannelSize(memQueue.size() + drainOrder.overflowCounter);
    channelCounter.stop();
    super.stop();
  }
