...
class BucketWriter {
  ...
  private volatile ScheduledFuture hourdRollFuture;
  ...
  private void open() throws IOException, InterruptedException {
    ...
    if (rollInterval > 0) {
      ...
      timedRollFuture = timedRollerPool.schedule(action, rollInterval,
        ...
        class HDFSRunnable implements Runnable{
        public void run(){
                    LOG.info("Rolling file ({}): Roll scheduled when entire hour.",
                        bucketPath, rollInterval);
                try {
                    close();
                } catch(Throwable t) {
       ...
       Runnable action = new HDFSRunnable();
       Calendar calendar = Calendar.getInstance();
       calendar.setTime(new Date());
       Long delaySec = new Long(calendar.get(Calendar.SECOND));
       Long delayMin = new Long(calendar.get(Calendar.MINUTE));
       delaySec = (70 - delayMin) * 60 - delaySec;
       hourdRollFuture = timedRollerPool.scheduleWithFixedDelay(action, delaySec, 3600,
          TimeUnit.SECONDS);
    }
  ...
  public synchronized void close(boolean callCloseCallback)
    ...
    if (rollInterval > 0) {
        ...
        if (hourdRollFuture != null && !hourdRollFuture.isDone()) {
            hourdRollFuture.cancel(false); // do not cancel myself if running!
            hourdRollFuture = null;
        }
    }
